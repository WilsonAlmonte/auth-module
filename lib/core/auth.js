...
export default class Auth {
  ...
  getExpiresIn (strategy) {
    const _key = this.options.expires_in.prefix + strategy

    return this.$storage.getUniversal(_key)
  }

  setExpiresIn (strategy, expiresIn) {
    const _key = this.options.expires_in.prefix + strategy

    return this.$storage.setUniversal(_key, expiresIn)
  }

  syncExpiresIn (strategy) {
    const _key = this.options.expires_in.prefix + strategy
  ...
  getOktaUserId (strategy) {
    const _key = this.options.okta_user_id.prefix + strategy
    return this.$storage.getUniversal(_key)
  }

  setOktaUserId (strategy, expiresIn) {
    const _key = this.options.okta_user_id.prefix + strategy
    return this.$storage.setUniversal(_key, expiresIn)
  }

  syncOktaUserId (strategy) {
    const _key = this.options.okta_user_id.prefix + strategy
    return this.$storage.syncUniversal(_key)
  }
